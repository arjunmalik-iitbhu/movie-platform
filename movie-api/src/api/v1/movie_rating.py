...
from src.dto import MovieRatingRes, MovieRatingCreateReq
...
async def read_rating(rating_id: str, session: AsyncSession = Depends(get_session)):
    result = await session.exec(select(MovieRating).where(MovieRating.id == int(rating_id)))
    rating = result.first()
    if not rating or not rating.id:
        raise HTTPException(status_code=404, detail=f"MovieRating {rating_id} not found")
...
async def create_rating(ratingReq: MovieRatingCreateReq, session: AsyncSession = Depends(get_session)):
